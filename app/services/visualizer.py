...
class VisualizerService:
    ...
    def generate_html_report(self, summary_data: Dict[str, Any], flow_graph_data: Dict[str, Any], output_path: str):
        ...
        rendered_html = template.render(
            ...
            flowchart_data=flow_graph_data.get("flowchart_data", {"nodes": [], "edges": []}),
            graph_data=flow_graph_data.get("graph_data", {"nodes": [], "edges": []})
        )
        
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(rendered_html)
            
        print(f"🎉 可视化 HTML 报告已更新生成: {Path(output_path).resolve()}")
